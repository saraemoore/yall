...
# use the optimal parameters selected using the tuning set to predict outcomes in the validation set
...
```{r}
# Validation set: Global feature selection by screening algorithm
...
```{r}
# Validation set: Global feature selection across all screening algorithms
#   (via SuperSelector package)
...
```{r}
# tidy(predall_holdout$predict$fitLibrary[[1]])
# predall_holdout$predict$kernel_weights[[30]] %>% as.vector()
# unname(predall_holdout$predict$library.predict) - Y_holdout
...
```{r}
# Validation set: Receiver operating characteristic curve
...
```{r}
# Validation set: Example Local Regression Model Coefficient Estimates for a single observation
...
local_mod <- predall_holdout$predict %>%
                filter(obsNum==as.character(holdout_obs_num)) %>%
                pull(fitLibrary) %>%
                `[[`(1)
